...
int main()
{
	...
	int n, arrayOriginalOrder[100005];
	int idSequence = 0, differentElements;
	struct nodeTree *z;
	while(scanf("%d", &n) != EOF)
    {
        z = TreeSearch(numeros, n);
        if(z == NULL){ //no se encontro el numero en el arbol
            numeros = TreeInsert(numeros,n);
            idSequence++;
            arrayOriginalOrder[idSequence] = n;
        }
        else{
            z->cant = z->cant + 1;
        }
	...
	differentElements = idSequence;

	for(idSequence = 1; idSequence <= differentElements;
        idSequence++)
    {
        z = TreeSearch(numeros, arrayOriginalOrder[idSequence]);
        printf("%d %d\n", z->key, z->cant);
    }
